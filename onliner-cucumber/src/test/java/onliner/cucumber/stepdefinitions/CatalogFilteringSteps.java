...
public class CatalogFilteringSteps {
    ...
    public void filterItemsByParameters(Map<String, String> filters) {
        ...
        filters.forEach((title, value) -> {
            switch (title.toLowerCase()) {
                case "producer" -> productPage.filterByProducer(value);
                case "max_price" -> productPage.filterByMaxPrice(value);
                case "min_diagonal" -> productPage.filterByMinDiagonal(value);
                case "max_diagonal" -> productPage.filterByMaxDiagonal(value);
                case "resolution" -> productPage.filterByResolution(value);
                default -> {}
            }
    ...
    public void checkCorrectSearchResults(DataTable table) {
        ...
        for (String title: titles) {
                switch (title.toLowerCase()) {
                    case "producer" -> softAssert.assertTrue(productPageFilterResults.isCorrectProducer(filterData.get("producer")),
                            "The \"Producer\" parameter does not match the search criteria.");
                    case "max_price" -> softAssert.assertTrue(productPageFilterResults.isCorrectPrice(filterData.get("max_price")),
                            "The \"Price\" parameter does not match the search criteria.");
                    case "diagonal" -> softAssert.assertTrue(productPageFilterResults.isCorrectDiagonal(filterData.get("min_diagonal"), filterData.get("max_diagonal")),
                            "The \"Diagonal\" parameter does not match the search criteria.");
                    case "resolution" -> softAssert.assertTrue(productPageFilterResults.isCorrectResolution(filterData.get("resolution")),
